...
namespace UserManagementSystem.Controllers
{
    ...
    public async Task<IActionResult> Register(RegisterViewModel model)
    {
        ...
        if (!IsPasswordValid(model.Password))
            {
                ModelState.AddModelError(nameof(model.Password), "Please enter a valid password (Validation requires at least one uppercase letter, one lowercase letter, one special character and one digit)");
                return View(model);
            }
            return View(model);
    }
        ...
        public async Task<IActionResult> ChangeEmail(ChangeEmailViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (model.EmailConfirmed != email)
                {
                    ...
                    TempData["EmailChanged"] = "Your Email Address has been successfully changed";
                    //email changed
                    return RedirectToAction(nameof(Index), "Profile");
                }
            }
